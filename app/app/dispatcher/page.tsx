...
export default function DispatchRequestDetail() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        // =================================================
        // LOAD RESPONDER PROFILES FIRST
        //
        // IMPORTANT:
        // We load responder_profiles first instead of
        // depending completely on users.role = responder.
        //
        // This allows responders to appear even when:
        // - users.role is missing/wrong
        // - users query is affected by RLS
        // - responder_profiles contains the responder
        // =================================================
        ...
        // =================================================
        // IF THERE ARE NO PROFILES, TRY USERS TABLE
        // =================================================
        ...
        let responderUsers: any[] = [];

        // If profiles exist, try to get matching users.
        if (profileUserIds.length > 0) {
          const {
            data: usersById,
            error: usersByIdError,
          } = await supabase
            .from("users")
            .select(`
              id,
          ...
          if (usersByIdError) {
            console.error(
              "Could not load responder users by ID:",
              usersByIdError
            );
          } else {
            responderUsers = usersById || [];
          }
        }

        // =================================================
        // FALLBACK:
        // TRY USERS WHERE ROLE = RESPONDER
        // =================================================

        if (responderUsers.length === 0) {
          const {
            data: roleResponders,
            error: roleRespondersError,
          } = await supabase
            .from("users")
            .select(`
              id,
          ...
          if (roleRespondersError) {
            console.error(
              "Could not load users with responder role:",
              roleRespondersError
            );
          } else {
            responderUsers =
        ...
        // =================================================
        // COMBINE RESPONDER PROFILES + USERS
        //
        // IMPORTANT:
        // Profiles are the primary source.
        // User information is optional.
        //
        // This means a responder profile can still appear
        // even if the users table query does not return
        // the matching user.
        // =================================================

        const combinedFromProfiles =
        ...
        //
        // This makes the page more tolerant if someone has
        // role = responder but their profile is missing.
        // =================================================
        ...
        // =================================================
        // FINAL RESPONDER LIST
        // =================================================

        const combinedResponders = [
          ...combinedFromProfiles,
  ...
  async function assignResponder() {
    ...
    if (availability !== "available") {
      alert(
        `This responder is currently ${
          availability || "unavailable"
        }. Please select an available responder.`
      );
            ...
            <label>
              ...
              0 ? (
                <p className="mt-2 text-sm text-red-500">
                  No responder profiles were
                  found in Supabase. Check the
                  responder_profiles table.
                </p>
              ) : (

...
async function getAuthenticatedUser() {
  ...
  if (!token) {
    console.log("AUTH: access_token cookie not found");
    return null;
  }
  ...
  if (!payload) {
    console.log("AUTH: access_token is invalid or expired");
    return null;
  }

  console.log("AUTHENTICATED USER:", payload.userId);
  console.log("AUTHENTICATED ROLE:", payload.role);

  return payload;
}

// ============================================================
// GET CURRENT ACTIVE RESPONDER MISSION
// ============================================================
  ...
  try {
    ...
    if (authUser.role !== "responder") {
      return NextResponse.json(
        {
          ok: false,
          message: "Only responders can access the responder mission.",
        },
        { status: 403 },
      );
    }
    ...
    console.log("==========================================");
    console.log("LOADING ACTIVE RESPONDER MISSION");
    console.log("RESPONDER USER:", authUser.userId);
    console.log("==========================================");

    // IMPORTANT:
    // Completed assignments are intentionally NOT included.
    const {
      data: assignment,
    ...
    if (assignmentError) {
      ...
      return NextResponse.json(
        {
          ok: false,
          message: "Unable to load responder assignment.",
        },
        { status: 500 },
      );
    }
    ...
    if (!assignment) {
      console.log("RESPONDER HAS NO ACTIVE ASSIGNMENT");
    ...
    console.log("ACTIVE ASSIGNMENT:", assignment.id);
    console.log("ACTIVE STATUS:", assignment.status);
    ...
    if (requestError) {
      ...
      return NextResponse.json(
        {
          ok: false,
          message: "Unable to load emergency request.",
        },
        { status: 500 },
      );
    }
    ...
    if (!emergencyRequest) {
      return NextResponse.json(
        {
          ok: false,
          message: "The assigned emergency request could not be found.",
        },
        { status: 404 },
      );
    }
    ...
    console.error("Responder mission GET error:", error);

    return NextResponse.json(
      {
        ok: false,
        message: "Unable to load responder mission.",
      },
      { status: 500 },
    );
  }
  ...
  try {
    ...
    if (authUser.role !== "responder") {
      return NextResponse.json(
        {
          ok: false,
          message: "Only responders can update responder missions.",
        },
        { status: 403 },
      );
    }
    ...
    if (assignmentError) {
      ...
      return NextResponse.json(
        {
          ok: false,
          message: "Unable to verify assignment.",
        },
        { status: 500 },
      );
    }
    ...
    if (status === "acknowledged") {
      assignmentUpdates.acknowledged_at = now;
    }

    if (status === "en_route") {
      assignmentUpdates.route_started_at = now;
    }

    if (status === "arrived") {
      assignmentUpdates.arrived_at = now;
    }
    ...
    if (updateAssignmentError) {
      ...
      return NextResponse.json(
        {
          ok: false,
          message: "Unable to update assignment status.",
        },
        { status: 500 },
      );
    }
    ...
    const {
      ...
      .insert({
        ...
        previous_status: previousRequestStatus,
    ...
    console.log(
      `MISSION UPDATED: ${assignment.status} -> ${status}`,
    );

    if (status === "completed") {
      console.log(
        "MISSION COMPLETED - IT WILL NO LONGER APPEAR AS ACTIVE",
      );
    }
    ...
    return NextResponse.json(
      {
        ok: false,
        message: "Unable to update responder mission.",
      },
      { status: 500 },
    );
  }
